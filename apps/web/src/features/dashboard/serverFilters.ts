...
export function applyClientFilters(
  ...
  return servers.filter(s => {
    ...
    if (filters.monuments && filters.monuments.length > 0) {
      const monumentNames = normalizeMonumentNames(s.monumentNames);
      if (monumentNames.length === 0) return false;
      const normalizedServerMonuments = new Set(
        monumentNames.map(m => normalizeMonumentName(m)).filter(Boolean)
      );
